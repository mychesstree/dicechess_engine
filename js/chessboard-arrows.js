...
var ChessboardArrows = function (
  ...
  function drawCircle(context, x, y, r, whichTry) {
    context.beginPath();
    context.lineWidth = 3;
    //context.arc(x, y, r, 0, 2 * Math.PI);

    // if (whichTry) {
    //   var p = primaryContext.getImageData(x, y, 1, 1).data;
    //   console.log(p);

    //   if (p[0] === 0) {
    //     context.fillStyle = "rgb(168 50 127)";
    //     context.fillRect(x - r - 1, y - r - 1, 44, 44);
    //   } else {
    //     drawContext.clearRect(x - r - 1, y - r - 1, 44, 44);
    //   }
    // } else {
      context.fillStyle = "rgb(168 50 127)";
      context.fillRect(x - r - 1, y - r - 1, 44, 44);
